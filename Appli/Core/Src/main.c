...
int main(void)
{
  ...
  /*** External RAM  *********************************************/
  rets = BSP_XSPI_RAM_Init(0);
  rets = BSP_XSPI_RAM_EnableMemoryMappedMode(0);

  test_data = *(volatile uint32_t*)0x90000000;
...
static void MPU_Config(void)
{
  ...
  default_config.BaseAddress = __PSRAM_NON_CACHEABLE_SECTION_BEGIN;
  default_config.LimitAddress =  __PSRAM_NON_CACHEABLE_SECTION_END;
  default_config.DisableExec = MPU_INSTRUCTION_ACCESS_DISABLE;
  default_config.AccessPermission = MPU_REGION_ALL_RW;
  default_config.IsShareable = MPU_ACCESS_NOT_SHAREABLE;
  default_config.AttributesIndex = MPU_ATTRIBUTES_NUMBER0;
